...
ws() {
  ...
  workspace_data="$(hypr_ipc j/workspaces)"
  current_workspace="$(hypr_ipc j/activeworkspace | jq -r '.id')"
  ...
  output="(box :class \"ws\" :halign \"end\" :orientation \"h\" :spacing 0 :space-evenly \"false\""
  for idx in "${!workspace_ids[@]}"; do
    i="${workspace_ids[$idx]}"
...
hypr_ipc() {
  printf '%s' "$1" | socat - "UNIX-CONNECT:$command_socket"
}
...
command_socket="$runtime_dir/hypr/$hypr_signature/.socket.sock"
event_socket="$runtime_dir/hypr/$hypr_signature/.socket2.sock"
[ -S "$command_socket" ] || exit 0
[ -S "$event_socket" ] || exit 0

ws
stdbuf -oL socat -U - "UNIX-CONNECT:$event_socket" | while read -r line; do
  case "$line" in
