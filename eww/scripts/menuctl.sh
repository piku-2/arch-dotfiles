...
get_cursor_eww_screen() {
    ...
    # Eww's `--screen` expects GDK monitor identifiers, which (on your setup)
    # match Hyprland's `model` field (e.g. "27D1Q", "KG272K L1"), NOT "DP-8".
...
window_id="menuctl_${safe_screen:-default}"

active_ids="$(/usr/bin/eww active-windows | awk -F': ' '$2 == "menuctl" {print $1}')"
...
# Close any menuctl instances open on other monitors
...
if [[ -n "${eww_screen}" ]]; then
    /usr/bin/eww open menuctl --id "${window_id}" --screen "${eww_screen}" && /usr/bin/eww update menurev=true
else
    /usr/bin/eww open menuctl --id "${window_id}" && /usr/bin/eww update menurev=true
fi
