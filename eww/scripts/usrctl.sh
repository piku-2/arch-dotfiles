...
window_id="usrctl_${safe_screen:-default}"

active_ids="$(/usr/bin/eww active-windows | awk -F': ' '$2 == "usrctl" {print $1}')"
...
if [[ -n "${eww_screen}" ]]; then
    /usr/bin/eww open usrctl --id "${window_id}" --screen "${eww_screen}" && /usr/bin/eww update ctlrev=true
else
    /usr/bin/eww open usrctl --id "${window_id}" && /usr/bin/eww update ctlrev=true
fi
