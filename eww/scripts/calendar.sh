...
window_id="calendar_${safe_screen:-default}"

active_ids="$(/usr/bin/eww active-windows | awk -F': ' '$2 == "calendar" {print $1}')"
...
if [[ -n "${eww_screen}" ]]; then
    /usr/bin/eww open calendar --id "${window_id}" --screen "${eww_screen}" && /usr/bin/eww update calrev=true
else
    /usr/bin/eww open calendar --id "${window_id}" && /usr/bin/eww update calrev=true
fi
